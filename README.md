...
気象庁（JMA）の数値予報モデル（MSM/GSM）をもとに、**東京の天気・気温・気圧の移り変わり（過去6時間〜12時間後）**をグラフ表示し、**気象病（天気痛）向けの気圧アラート**を出す静的Webアプリです。バックエンド不要で、ブラウザから直接データを取得します。
...
気象病は「気圧の絶対値」よりも **急な気圧の低下（下降速度）** が引き金になりやすいとされます。本アプリは今後12時間の気圧予報から **3時間あたりの最大の低下幅** を求め、4段階の警戒レベルを最上部に表示します。

| レベル | 3時間あたりの低下 | 目安 |
| --- | --- | --- |
| 🟢 安定 | 〜1.5 hPa | 影響は小さい |
| 🟡 やや注意 | 1.5〜3 hPa | 敏感な方は注意 |
| 🟠 注意 | 3〜5 hPa | 休息・服薬の準備を |
| 🔴 警戒 | 5 hPa〜 | 早めの対策を |

「何時頃に約何hPa下がるか」も併記し、気圧グラフ上の該当時間帯を帯で強調します。
※ 体調管理の目安であり、診断・治療を行うものではありません。閾値は `app.js` の `analyzePressure` / `ALERT_LEVELS` で調整できます。
